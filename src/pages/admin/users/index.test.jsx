...
const mockReadersShort = { data: [{ id: 1, username: 'reader1', joinDate: '2024-01-01' }], total: 1 };
const mockWritersShort = { data: [{ id: 2, username: 'writer1', joinDate: '2024-02-01' }], total: 1 };
const mockReadersAll = { data: [{ id: 1, username: 'reader1', status: 'active' }], total: 1 };
const mockWritersAll = { data: [{ id: 2, username: 'writer1', status: 'active' }], total: 1 };
...
describe('UsersOverview', () => {
  beforeEach(() => {
    ...
    userService.getReaders.mockResolvedValueOnce(mockReadersShort).mockResolvedValueOnce(mockReadersAll);
    userService.getWriters.mockResolvedValueOnce(mockWritersShort).mockResolvedValueOnce(mockWritersAll);

    analyticsService.getPlatformDAU.mockResolvedValue({ success: true, data: { dau: 10, wau: 20, mau: 30, hourlyBreakdown: [] } });
    rankingService.getUserRankings.mockResolvedValue({ success: true, data: { content: [] } });
    rankingService.getAuthorRankings.mockResolvedValue({ success: true, data: { content: [] } });
  });
