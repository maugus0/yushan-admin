...
const generateMockReviews = () => {
  ...
  for (let novelId = 1; novelId <= 100; novelId++) {
    ...
    for (let i = 0; i < reviewCount; i++) {
      ...
      const rating =
        Math.random() > 0.1 // 90% positive reviews
          ? Math.random() > 0.3
            ? 5
            : 4 // 70% are 5-star, 20% are 4-star
          : Math.random() > 0.5
            ? 3
            : Math.random() > 0.5
              ? 2
      ...
      reviews.push({
        ...
        sentimentScore:
          rating >= 4
            ? Math.random() * 0.4 + 0.6 // Positive
            : rating === 3
              ? Math.random() * 0.6 - 0.3 // Neutral
...
export const reviewService = {
  ...
  updateReview: async (id, updateData) => {
    try {
      ...
      if (
        (updateData.content && updateData.content !== originalReview.content) ||
