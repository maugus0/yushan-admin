...
const generateMockComments = () => {
  const comments = [];
  const usernames = [
    'bookworm_2024', 'novel_lover', 'fantasy_fan', 'romance_reader', 'action_seeker',
    'mystery_solver', 'sci_fi_explorer', 'horror_enthusiast', 'drama_queen', 'adventure_time',
    'reader_extraordinaire', 'story_hunter', 'chapter_chaser', 'plot_analyzer', 'character_critic'
  ];
  ...
  for (let novelId = 1; novelId <= 50; novelId++) {
    ...
    for (let i = 0; i < novelCommentCount; i++) {
      ...
      const parentComment = isReply ? comments[Math.floor(Math.random() * Math.min(comments.length, 50))] : null;
      ...
      comments.push({
        ...
        editedAt: Math.random() > 0.9 ? new Date(createdDate.getTime() + Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString() : null,
      });
    }
  }
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          engagementRate: ((periodComments.reduce((sum, c) => sum + c.likes, 0) / periodComments.length) || 0).toFixed(1),
        }
      };
    } catch (error) {
