...
const generateMockNovels = () => {
  ...
  const authors = [
    'Sarah Connor', 'Michael Zhang', 'Elena Rodriguez', 'James Park', 'Luna Kim',
    'Alex Chen', 'Grace Li', 'David Kim', 'Maria Santos', 'Lisa Wang',
    'Robert Johnson', 'Amy Wu', 'Kevin Lee', 'Sophie Chen', 'Daniel Liu'
  ];
  
  const novelTitles = [
    'Dragon Realm Chronicles', 'Mystic Sword Master', 'Love in Ancient Times', 
    'Cyber Ninja Academy', 'Eternal Phoenix', 'Shadow Hunter Guild',
    'Moonlight Serenade', 'Quantum Legends', 'Sacred Beast Tamer',
    'Imperial Court Romance', 'Starfall Warriors', 'Crystal Heart Saga',
    'Demon King\'s Bride', 'Azure Sky Pavilion', 'Crimson Blade Dance',
    'Heavenly Immortal Path', 'Dark Moon Rising', 'Golden Phoenix Empire',
    'Silver Wolf Chronicles', 'Jade Mountain Secrets', 'Thunder God\'s Legacy',
    'Frozen Heart Kingdom', 'Burning Soul Technique', 'Wind Spirit Master',
    'Earth Shaking Fist', 'Water Dragon King', 'Fire Phoenix Goddess',
    'Lightning Sword Saint', 'Ice Queen\'s Domain', 'Steel Warrior\'s Path'
  ];
  
  return Array.from({ length: 150 }, (_, i) => {
    const createdDate = new Date(2023, Math.floor(Math.random() * 24), Math.floor(Math.random() * 28) + 1);
    const updatedDate = new Date(createdDate.getTime() + Math.random() * (Date.now() - createdDate.getTime()));
    const status = statuses[Math.floor(Math.random() * statuses.length)];
    
    return {
      id: i + 1,
      title: novelTitles[i % novelTitles.length] + (i >= novelTitles.length ? ` ${Math.floor(i / novelTitles.length) + 1}` : ''),
      slug: (novelTitles[i % novelTitles.length] + (i >= novelTitles.length ? ` ${Math.floor(i / novelTitles.length) + 1}` : '')).toLowerCase().replace(/[^a-z0-9]+/g, '-'),
      ...
      translatedBy: Math.random() > 0.7 ? authors[Math.floor(Math.random() * authors.length)] : null,
      ...
      // Publishing
      publishedAt: status === 'published' || status === 'completed' ? createdDate.toISOString() : null,
      lastChapterAt: status === 'published' || status === 'completed' ? updatedDate.toISOString() : null,
      scheduledAt: null,
...
export const novelService = {
  ...
  getAllNovels: async (params = {}) => {
    try {
      ...
      if (author) {
        novels = novels.filter(novel => novel.author.toLowerCase().includes(author.toLowerCase()));
      }
