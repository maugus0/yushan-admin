...
const generateMockChapters = () => {
  const chapters = [];
  const chapterTitles = [
    'The Beginning', 'First Encounter', 'Hidden Powers', 'The Journey Starts',
    'Unexpected Allies', 'Dark Secrets', 'Training Arc', 'The Tournament',
    'Betrayal', 'Rising Storm', 'Final Battle', 'New Dawn', 'Mystery Unveiled',
    'Power Awakening', 'The Chosen One', 'Ancient Prophecy', 'Lost Memories',
    'Forbidden Love', 'Epic Confrontation', 'The Resolution'
  ];
  ...
  for (let novelId = 1; novelId <= 30; novelId++) {
    ...
    for (let chapterNum = 1; chapterNum <= chapterCount; chapterNum++) {
      const publishDate = new Date(2023, Math.floor(Math.random() * 12), Math.floor(Math.random() * 28) + 1);
      ...
      chapters.push({
        ...
        authorNotes: Math.random() > 0.7 ? `Author's note for chapter ${chapterNum}: Thank you for reading!` : null,
        ...
        scheduledAt: null,
        lastEditedAt: new Date(publishDate.getTime() + Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(),
        ...
        updatedAt: new Date(publishDate.getTime() + Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(),
      });
    }
  }
  ...
  createChapter: async (chapterData) => {
    try {
      ...
      const newChapter = {
        ...
        slug: chapterData.title ? chapterData.title.toLowerCase().replace(/[^a-z0-9]+/g, '-') : `chapter-${maxChapterNumber + 1}`,
        ...chapterData,
        wordCount: chapterData.content ? chapterData.content.split(/\s+/).length : 0,
        readingTime: chapterData.content ? Math.ceil(chapterData.content.split(/\s+/).length / 200) : 0,
        views: 0,
        uniqueViews: 0,
        likes: 0,
        comments: 0,
