...
const generateMockRankingData = () => {
  ...
  const genres = [
    'fantasy', 'romance', 'action', 'sci-fi', 'mystery', 'horror', 'drama',
    'comedy', 'adventure', 'historical', 'slice-of-life', 'thriller'
  ];
  ...
  rankingTypes.forEach(rankingType => {
    ...
    const globalRanking = {
      ...
      genre: null,
      region: 'global',
      timeframe: rankingType.updateFrequency === 'hourly' ? '1h' :
                 rankingType.updateFrequency === 'daily' ? '24h' :
                 rankingType.updateFrequency === 'weekly' ? '7d' : '30d',
      entries: [],
      totalEntries: 100,
    ...
    for (let position = 1; position <= 100; position++) {
      const entry = {
        ...
        lastPositionChange: new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(),
...
export const rankingService = {
  ...
  getTrendingNovels: async (params = {}) => {
    try {
      ...
      let trendingRanking;
      if (genre) {
        trendingRanking = mockRankings.find(r => 
          r.type === 'trending' && 
          r.genre === genre
        );
      } else {
  ...
  getPopularNovels: async (params = {}) => {
    try {
      ...
      let popularRanking;
      if (genre) {
        popularRanking = mockRankings.find(r => 
          r.type === 'popular' && 
          r.genre === genre &&
  ...
  getRankingStats: async () => {
    try {
      ...
      const averageEntries = mockRankings.reduce((sum, r) => sum + r.totalEntries, 0) / totalRankings;
      ...
      return {
        ...
        data: {
          ...
          needsUpdate: mockRankings.filter(r => 
            new Date(r.nextUpdate) <= new Date()
          ).length,
        }
      };
    } catch (error) {
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          averagePosition: Math.round(history.reduce((sum, h) => sum + h.position, 0) / history.length),
        },
      };
    } catch (error) {
  ...
  getRankingTypes: async () => {
    try {
      ...
      const types = [
        ...
        { id: 'completed', name: 'Recently Completed', description: 'Novels that finished recently' },
        ...
        { id: 'most_commented', name: 'Most Commented', description: 'Novels with most discussion' },
